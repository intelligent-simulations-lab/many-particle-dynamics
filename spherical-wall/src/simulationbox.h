...
class SIMULATIONBOX
{
  ...
  double radius;        // Radius of the bouncing sphere
  
  // put particles inside the box
  void put_ljatoms_inside(vector<PARTICLE>&, int, double, double, double);
  
  SIMULATIONBOX(VECTOR3D m_posvec = VECTOR3D(0,0,0), double m_lx = 10.0, double m_ly = 10.0, double m_lz = 10.0, double r= 5.0)//7.071067)
  {
    posvec = m_posvec;
    lx = m_lx;
    ly = m_ly;
